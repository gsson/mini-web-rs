...
use crate::json_logging::fields::FieldRecorder;
        ...
        W: for<'writer> MakeWriter<'writer> + 'static,
{
    fn on_new_span(&self, attrs: &Attributes<'_>, id: &Id, ctx: Context<'_, S>) {
        ...
        if extensions.get_mut::<FieldRecorder>().is_none() {
            extensions.insert(FieldRecorder::from_attributes(&self.event_format, attrs));
        }
    }
    ...
    fn on_record(&self, id: &Id, record: &Record<'_>, ctx: Context<'_, S>) {
        ...
        if let Some(fields) = extensions.get_mut::<FieldRecorder>() {
